...
function rowDbclick(r){
	$(r).dblclick(
	 function() {
		 ...
		 $("#locationLogId").val(columnsArray[0]);
		 $("#name").val(columnsArray[1]);
		 $("#lon").val(columnsArray[2]);
		 $("#lat").val(columnsArray[3]);
		 $("#city").val(columnsArray[4]);
		 $("#logDatetime").val(columnsArray[5]);
		 $("#updateMapLogDailDialog").dialog("open");
	 });
...
$(function()
{
	...
	$("#flexiGridID").flexigrid({
		width : _gridWidth,
		height : _gridHeight,
		url : "${pageContext.request.contextPath}/mapLog/getMapLogList",
		dataType : 'json',
		colModel : [ 
	       {display : '日志编号',name : 'locationId',width : _columnWidth, sortable : true,align : 'center',hide : 'true'}, 
	...
	$("#delBtn").click(function( event ) {
		...
		if(ids.length > 0){
			$.ajax({
				type : 'POST',
				url : '${pageContext.request.contextPath}/mapLog/deleteMapLog',
				dataType : 'json',
				cache : false,
				data : [ {
					name : 'mapIds',
	...
	$('#updateMapLogDailFormId').ajaxForm({
		dataType: "json",
		success : function(data) {
			$('#flexiGridID').flexReload();
			$("#updateMapLogDailDialog").dialog( "close" )
	     },
	     error : function() {
	        message("请联系管理员");
	     },
...
<label> 监测点名称:</label> <input type="text" id="searchName"/>
