...
(function($) {
    $.addFlex = function(t, p) {
        ...
        var g = {
            ...
            rePosDrag: function() {
                ...
                $('thead tr:first th:visible', this.hDiv).each(function() {
                    ...
                    // in iframe and ie  update by chenqi
            ...
            fixHeight: function(newH) {
                ...
                //modify by zhaokai 解决选择显示列的时候，有时候不出滚动条的问题。
                if (nd >= newH && newH != 0)
            ...
            toggleCol: function(cid, visible) {
                ...
                if (visible == null) {
                	//增加判断如果隐藏属性第一次没有值，默认取新增的用于列显示的隐藏属性 modify by libiao
                	if(ncol.hide == null){
            ...
            addData: function(data) { //parse data   
                ...
                //增加判断不分页时不修改分页栏 modify by zhaokai
            	if (p.usepager){
                ...
                if (p.total == 0) {
                	...
                	//重新设置表头文字 begin ouyang 20141120
                ...
                if(p.appendData) {//增加向表格追加行判断 modify by ouyang 20141016
                ...
                if (p.dataType == 'json') {
                    if (data.rows != null) {
                        $.each(data.rows, function(i, row) {
                            ...
                            if (p.rowbinddata) {
                            			//.replace("'",'&apos;').replace('"','&amp;') modify by ouyang 20140917
                            ...
                            $(ths).each(function(j) {
                                ...
                                //如果用于隐藏或用于刚开始隐藏后面列可选择显示，隐藏列  modify by libiao
				 	    ...
				 	    $(ths).each(function(j) {
				 	        ...
				 	        if (idx == "-1") { //checkbox
                            	...
                            	if(p.showcheckbox){
				 	        	...
				 	        	//如果是单选列表，显示单选框 modify by zhaokai
                            	else if(p.showradio){
                ...
                //增加向表格追加行判断 modify by ouyang 20141016
                if(_createTbody){
                	tbhtml.push("</tbody>");
                	$(t).html(tbhtml.join(""));
                }else{
            ...
            populate: function() { //get latest data 
                ...
                if (p.onSubmit) {
                    var gh = p.onSubmit();
                    if (!gh){
                    	//设置loading为false解决表格不查询问题  modify by zhaokai
                    	this.loading = false;
                    	return false;
                    }
                }
                    ...
                    function() {
                        $("input.itemchk", this).each(function() {
                            ...
                            $(this).click(function() {
                                ...
                                //增加判断是否所有checkbox都点击的，如果是，全选自动勾选   add by zhaokai
                    	    	if(this.checked)
                    	    	{
        ...
        if (p.colModel) {
            ...
            for (i = 0; i < p.colModel.length; i++) {
                ...
                //modify by zhaokai 记录列中的类型和操作信息
                if(cm.optType){
                ...
                //增加判断如果是用于列选择的隐藏，先隐藏 add by libiao
                if(cm.hideForSelect){
        ...
        $('thead tr:first th', g.hDiv).each
			(
			 	function() {
			 	    ...
			 	    //增加判断用于列选择的隐藏列初始化隐藏  modify by libiao
			 	    ...
			 	    //p.isDragCol增加判断拖拽表头排序 modify by ouyang 20141015
						...
						.hover(
							function() {
							    ...
							    if (g.colCopy) {
							        ...
							        //增加列选择控制  add by zhaokai
							        if(p.isColSel)
							        {	//modify by ouyang 20140819 解决chrome中隐藏列按钮太长问题
							...
							function() {
							    ...
							    //modify by zhaokai 增加判断 typeof(p.sortname) != 'undefined'，解决没有排序的列鼠标放上面会出箭头的问题
        ...
        if (p.usepager) {
            ...
            $('div', g.pDiv).html(html);
 			 ...
 			 //此功能添加go图标进行点击操作 start, author:龚壮 2013-01-30
            $('.pReloadGo', g.pDiv).click(function() { g.changePage('input');});
            //此功能添加go图标进行点击操作end, author:龚壮 2013-01-30
        ...
        if ($('th', g.hDiv).length) {
			...
			(
			 	function() {
			 	    ...
			 	    else{//begin 解决单选时隐藏列出问题 add by ouyang
			 	    ...
			 	    if(typeof(kcol.hide) != 'undefined' || kcol.hide == true){
			 	    	...
			 	    	//modify by zhaokai 去掉样式 noborder，解决checked不起作用的问题
    ...
    //向表格追加数据 modify by ouyang 20141016
    ...
    //添加json数据 modify by bianxw 20150306
